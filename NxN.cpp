...
{
  if (i >= 0 && j >= 0 && i < TAMANHO_DO_TABULEIRO && j < TAMANHO_DO_TABULEIRO)
    return true;
  return false;
}
...
bool isNotAttacked(int i, int j, vector<vector<bool>> table)
{
  ...
  {
    if (table[i][x])
      return false; // ve se tem alguem na msm linha
  }
  // diagonal principal
...
void recursion(int i, vector<int> &queens, vector<vector<int>> &solutions, vector<vector<bool>> &table)
{ // i = numero da rainha, top = ja tentei todas as posições para essa rainha
  if (i == TAMANHO_DO_TABULEIRO)
    solutions.push_back(queens); // fora do tabuleiro
  else if (queens[i] != -1)
...
int main()
{
  // Tá fazendo chamar o CIN antes do COUT
